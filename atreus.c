...
void calculate_presses() {
  ...
  for(int i = 0; i < pressed_count; i++) {
    ...
    if(keycode >= MIN_FUNCTION && keycode <= MAX_FUNCTION) {
      ...
      // layer disabled, no keypress processed
      ...
      // if first_click_key is defined, is the only key allowed
      if (first_click_key == 0 || (keycode & 255) == first_click_key) {
        keyboard_keys[usb_presses++] = (keycode & 255);
      }
    } else if(usb_presses < 6){
      // keypress
      // if first_click_key is defined, is the only key allowed
      if (first_click_key == 0 || keycode == first_click_key) {
...
void set_leds() {
  if (current_layer != previous_layer) {
    ...
    } else if (current_layer == layers[2]) {
      DDRB  |=  (1 << DDB3);
